...
public final class UpdateSignatureValuePresenter {
    private let updateSignatureValue: UpdateSignatureValue
    private let alertView: AlertView
    private let loadingView: LoadingView
    private let validation: Validation
    private let viewModel: ResultNoContentViewModel
    
    public init(updateSignatureValue: UpdateSignatureValue,
                alertView: AlertView,
                loadingView: LoadingView,
                validation: Validation,
                viewModel: ResultNoContentViewModel) {
        self.updateSignatureValue = updateSignatureValue
    ...
    public func update(request: UpdateSignatureValueRequest, and pathComponent: PathComponentRequest) {
        if let message = validation.validate(data: request.toJson()) {
            ...
            updateSignatureValue.update(with: request.toUpdateSignatureValueParemeters(), to: pathComponent.toPathComponent()) { [weak self] result in
